...
void rtmp_client::do_handshake_c0c1(boost::function<void(const boost::system::error_code)> ready_handler)
{
    ...
    // write c0,c1
    boost::asio::async_write(socket_, boost::asio::buffer(rtmp_output_stream_.data(),
            rtmp_output_stream_.size()),
            [this, ready_handler](boost::system::error_code err, size_t bytes_transferred) {
                if (!err) {
                    BOOST_LOG_TRIVIAL(info) << "RTMP Handshake c0,c1 sent success! size:" << bytes_transferred << "\n";
                    // read s0,s1,s2
                    boost::asio::async_read_until(socket_, read_stream_,
                            rtmp_s1_matcher(1+RTMP_S1_LENGTH+RTMP_S2_LENGTH),
                            [this, ready_handler](boost::system::error_code err,
                                    size_t bytes_transferred) {
                                if (!err) {
                                    BOOST_LOG_TRIVIAL(info)
                                        << "RTMP Handshake s0,s1,s2 read success ! size:" << bytes_transferred << "\n";
                                    const char* data_ptr = boost::asio::buffer_cast<const char*>(
                                            this->read_stream_.data());
                                    rtmp_output_stream_.create_c2_packet(data_ptr+1, RTMP_S1_LENGTH);
                                    this->read_stream_.consume(bytes_transferred);
                                    boost::asio::async_write(socket_, boost::asio::buffer(
                                            rtmp_output_stream_.data(), rtmp_output_stream_.size()),
                                            [this, ready_handler](boost::system::error_code err,
                                                    size_t bytes_transferred) {
                                                if (!err) {
                                                    BOOST_LOG_TRIVIAL(info) << "Handshake c2 send success !"
                                                                            << bytes_transferred << "\n";
                                                }
                                                else {
                                                    BOOST_LOG_TRIVIAL(error)
                                                        << "Handshake c2 send failed!" << err.message() << "\n";
                                                }
                                                ready_handler(err);
                                            });
                                }
...
void rtmp_client::do_rtmp_connect(rtmp_cmd_connect_t& cmd,
        boost::function<void(const boost::system::error_code)> ready_handler)
{
    rtmp_output_stream_.create_connect_packet(cmd);
    boost::asio::async_write(socket_, boost::asio::buffer(rtmp_output_stream_.data(), rtmp_output_stream_.size()),
            [this, ready_handler](boost::system::error_code err, size_t bytes_transferred) {
                if (!err) {
                    BOOST_LOG_TRIVIAL(info) << "do_rtmp_connect send success!" << bytes_transferred << "\n";
                    boost::asio::async_read_until(socket_, read_stream_, rtmp_chunk_matcher(RTMP_S1_LENGTH),
                            [this, ready_handler](boost::system::error_code err, size_t bytes_transferred) {
                                if (!err) {
                                    ...
                                    BOOST_LOG_TRIVIAL(error) << "do_rtmp_connect read err!"
                                                             << socket_.remote_endpoint() << err.message() << "\n";
                                }
                            });
                }
                else {
