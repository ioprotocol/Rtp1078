...
void rtmp_client::do_handshake_c0c1(boost::function<void(const boost::system::error_code)> ready_handler) {
    ...
    boost::asio::async_write(socket_,
                             ...
                             [this, ready_handler](boost::system::error_code err, size_t bytes_transferred) {
                                 if(!err) {
                                    BOOST_LOG_TRIVIAL(info) << "Handshake c0 and c1 has sent!\n";
                                    
                                 } else {
                                    BOOST_LOG_TRIVIAL(info) << "Handshake c0 and c1 send err!" << err.message() << "\n";
                                    ready_handler(err);
                                 }
                             });
}
...
void rtmp_client::do_handshake_c2(boost::function<void(const boost::system::error_code)> ready_handler) {
