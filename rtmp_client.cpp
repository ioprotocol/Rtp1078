...
void rtmp_client::do_receive_rtmp_packet()
{
	boost::asio::async_read_until(socket_, read_stream_, rtmp_chunk_matcher(chunk_size_), [this](boost::system::error_code err, size_t bytes_transferred)
	{
		...
		{
			const char* data_ptr = boost::asio::buffer_cast<const char*>(this->read_stream_.data());
			do_parse_rtmp_packet(data_ptr, bytes_transferred, [this](boost::system::error_code err) {
...
void rtmp_client::do_parse_rtmp_packet(const char* buf, std::size_t size, boost::function<void(const boost::system::error_code)> handler)
{
	...
	case NGX_RTMP_MSG_CHUNK_SIZE:
		do_handle_rtmp_set_chunk_size(buf, size, handler);
		break;
	...
	case NGX_RTMP_MSG_ACK_SIZE:
		do_handle_rtmp_set_ack_size(buf, size, handler);
		break;
	case NGX_RTMP_MSG_BANDWIDTH:
		do_handle_rtmp_set_bandwidth(buf, size, handler);
		break;
...
//	t.expires_from_now(boost::posix_time::seconds(25));
//	t.async_wait(handler);

	read_stream_.consume(size);

	if (receive_bytes_count_ > acknowledgement_size_)
	{
		this->do_send_acknowledgement(handler);
	}
}

void rtmp_client::do_handle_rtmp_set_ack_size(const char* buf, std::size_t size, boost::function<void(const boost::system::error_code)> handler)
{
...
void rtmp_client::do_handle_rtmp_set_bandwidth(const char* buf, std::size_t size, boost::function<void(const boost::system::error_code)> handler)
{
	uint32_t head_size = rtmp_header_size(*buf);
	...
	this->do_send_acknowledgement_size(handler);
}

void rtmp_client::do_handle_rtmp_set_chunk_size(const char* buf, std::size_t size, boost::function<void(const boost::system::error_code)> handler)
{
	uint32_t head_size = rtmp_header_size(*buf);
...
void rtmp_client::do_send_acknowledgement(boost::function<void(const boost::system::error_code)> handler)
{
	...
	boost::asio::async_write(socket_, boost::asio::buffer(rtmp_output_stream_.data(), rtmp_output_stream_.size()), [this, handler](boost::system::error_code err, size_t bytes_transferred)
	{
		receive_bytes_count_ = 0;
		if (err)
		{
...
void rtmp_client::do_send_acknowledgement_size(boost::function<void(const boost::system::error_code)> handler)
{
